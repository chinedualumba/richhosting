...
<?php
	...
	function widget($args, $instance) {
		extract($args);
			...
			$consumer_key = dttheme_option('general','bottombar-twitter-key');
			$consumer_secret = dttheme_option('general','bottombar-twitter-secret');
			$access_token = dttheme_option('general','bottombar-twitter-token');
			$access_token_secret = dttheme_option('general','bottombar-twitter-token-secret');
		...
		echo !empty( $before_widget ) ? $before_widget : '';			
			...
			if ( !empty( $title ) ) {
				...
				echo "<a href='http://twitter.com/$username/' title='".strip_tags($title)."'>".$title ."</a>";
				...
				if($twitter && is_array($twitter)) {
					foreach( $twitter as $tweet ):
						...
						echo '<li class="tweet">';
								...
								if( $time )
								echo "<span class='tweet-time'>".esc_html( $timeAgo )."</span> ";
						
								if( $display_avatar )
								echo '<span class="tweet-thumb"><a href="http://twitter.com/'.$username.'" title=""><img src="'.esc_url( $tweet->user->profile_image_url ).'" alt="" /></a></span>';
