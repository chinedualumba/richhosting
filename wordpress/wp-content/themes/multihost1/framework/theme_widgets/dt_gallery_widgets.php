	...
	function form($instance) {
	    ...
	    <p><label for="<?php echo esc_attr( $this->get_field_id('_post_categories') ); ?>">
            ...
            <select class="widefat" id="<?php echo esc_attr( $this->get_field_id('_post_categories') ).'[]';?>" 
           	...
           	<?php $cats = get_categories('taxonomy=dt_portfolio_entries&hide_empty=1');
			foreach ($cats as $cat):
        ...
        <p><label for="<?php echo esc_attr( $this->get_field_id('_excerpt') ); ?>"><?php _e('Display title only or title &amp; excerpt','dt_themes');?></label>
		   ...
		   <?php foreach ($answers  as $answer ): 
                  ...
                  <option <?php echo($selected);?> value="<?php echo esc_attr( $answer );?>"><?php echo esc_html($answer);?></option>
           <?php endforeach; ?>
           </select></p>
	    ...
	    <p><label for="<?php echo esc_attr( $this->get_field_id('_post_count') ); ?>"><?php _e('No.of posts to show:','dt_themes');?></label>
		   <input id="<?php echo esc_attr( $this->get_field_id('_post_count') ); ?>" name="<?php echo esc_attr( $this->get_field_name('_post_count') ); ?>" value="<?php echo esc_html($_post_count);?>" /></p>
        <!-- Form end-->
<?php
	}
	...
	function widget($args, $instance) {
		...
		$arg = empty($_post_categories) ? $arg : array(
											...
											'tax_query'		=> array(array( 'taxonomy'=>'dt_portfolio_entries', 'field'=>'id', 'operator'=>'IN', 'terms'=>$_post_categories ) ));
		echo !empty( $before_widget ) ? $before_widget : '';
		
		echo !empty( $title ) ? $before_title.$title.$after_title : $before_title.$after_title;
		echo "<div class='recent-dt-gallery-widget'><ul>";	
			...
			if( $the_query->have_posts()) :
				while($the_query->have_posts()):
					...
					if($show_title):
						echo "<h6><a href='".get_permalink()."'>".esc_html( $title )."</a></h6>";
					else:
						echo "<h6><a href='".get_permalink()."'>".esc_html( $title )."</a></h6>";
