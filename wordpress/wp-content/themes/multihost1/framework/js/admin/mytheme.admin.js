...
var mythemeAdmin = {
  ...
  layoutSelect : function(){
    jQuery("#page-layout").find("a").click(function(e){
      ...
      if( jQuery(".sidebar-section").length ) {

        if( $rel == "content-full-width" ) {
          jQuery(".widget-area-options").slideUp();
        } else if( $rel == "with-left-sidebar" || $rel == "with-right-sidebar" ) {
          jQuery(".widget-area-options").slideDown();
        }
      }
      e.preventDefault();
    });
    ...
    jQuery("a.dttheme-import-button").click(function(e){
      if(jQuery(this).hasClass('import-disabled')) {
        ...
        jQuery.ajax({
          type:"POST",
          url:ajaxurl,
          data:{action:'mytheme_ajax_importer', 'data': $data },
    ...
    jQuery("a.mytheme-import-button").bind('click',function(e){
      ...
      if(jQuery(this).hasClass('import-disabled')) {
        ...
        if(confirm(objectL10n.importConfirm)){
          ...
          var $dummy =  'multihost'; // replace this line with bellow line and enable the line no : 336
