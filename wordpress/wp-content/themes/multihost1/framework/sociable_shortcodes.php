...
function stumbleupon( $attrs = null, $content = null,$shortcodename ="" ){
	...
	$url = "&r='".esc_js($url)."'";
	$out = '<script src="http://www.stumbleupon.com/hostedbadge.php?s='.esc_js($layout).$url.'"></script>';
	return $out;	
}
...
function linkedin( $attrs = null, $content = null,$shortcodename ="" ){
	extract(shortcode_atts(array('layout'=>'2','url'=>get_permalink()),$attrs));
    	...
    	if ($url != '') { $url = "data-url='".esc_js($url)."'"; }
	    if ($layout == '2') { $layout = 'right'; }
		if ($layout == '3') { $layout = 'top'; }
		$out = '<script type="text/javascript" src="http://platform.linkedin.com/in.js"></script><script type="in/share" data-counter = "'.esc_js($layout).'" '.$url.'></script>';
	return $out;	
}
...
function dt_sc_social($attrs, $content=null,$shortcodename="") {
	...
	if( isset($dttheme_options['general']['show-sociables']) && isset($dttheme_options['social']) ):
		...
		foreach($dttheme_options['social'] as $social) :
			$class = str_replace("fa-", "", $social['icon']);
			$out .= '<li class="'.$class.'">';
			$out .= '<a class="fa '.$social['icon'].'" href="'.$social['link'].'"></a>';
			$out .= "</li>";
		endforeach;
		$out .= "</ul>";
		return $out;
